...
static const char *TAG = "PKCS11+PAL";
                                        ...
                                        CK_ULONG usLength )
{
    ...
    prvLabelToFilenameHandle( pxLabel,
                              &pcFileName,
                              &xHandle );
    //ESP_EARLY_LOGI( TAG, "FIND OBJECT %s", pcFileName );
...
{
    initialize_nvs_partition();
    //ESP_EARLY_LOGI( TAG, "GET_OBJECT_VALUE");

    char * pcFileName = NULL;
    CK_RV ulReturn = CKR_OK;
    // ESP_EARLY_LOGI( TAG, "GET_OBJECT_VALUE %s", pcFileName );

    if( xHandle == eAwsDeviceCertificate )
    {
        pcFileName = pkcs11palFILE_NAME_CLIENT_CERTIFICATE;
        *pIsPrivate = CK_FALSE;
        //ESP_EARLY_LOGI( TAG, "DEVICE_CERTI_AWS %s", pcFileName );
    }
    else if( xHandle == eAwsDevicePrivateKey )
    {
        pcFileName = pkcs11palFILE_NAME_KEY;
        *pIsPrivate = CK_TRUE;
        //ESP_EARLY_LOGI( TAG, "DEVICE_PRIVATE_AWS %s", pcFileName );
    }
    else if( xHandle == eAwsDevicePublicKey )
    {
        ...
        pcFileName = pkcs11palFILE_NAME_KEY;
        *pIsPrivate = CK_FALSE;
        //ESP_EARLY_LOGI( TAG, "DEVICE_PUBLIC_AWS %s", pcFileName );
    }
    else if( xHandle == eAwsCodeSigningKey )
    {
        pcFileName = pkcs11palFILE_CODE_SIGN_PUBLIC_KEY;
        *pIsPrivate = CK_FALSE;
        //ESP_EARLY_LOGI( TAG, "DEVICE_CODE_SIGNING_AWS %s", pcFileName );
    }
    ...
    if (ulReturn == CKR_OK)
    {
       // ESP_EARLY_LOGI(TAG, "Reading file %s", pcFileName);
        nvs_handle handle;
        esp_err_t err = nvs_open_from_partition(NVS_PART_NAME, NAMESPACE, NVS_READONLY, &handle);
        if (err != ESP_OK) {
            /* This can happen if namespace doesn't exist yet, so no files stored */
